...
We prepare data from two sources, corresponding to an in-distribution training/evaluation benchmark and three out-of-distribution (OOD) evaluation datasets.

\paragraph{LoCoMo (In-Distribution).}
We use the LoCoMo benchmark~\citep{maharana2024evaluating}, a long multi-session dialogue dataset consisting of 10 conversations ($\sim$600 turns, $\sim$26k tokens each) annotated with QA pairs spanning single-hop, multi-hop, open-domain, and temporal reasoning. We adapt LoCoMo for both flat and structured memory:
\begin{itemize}[nosep]
    \item \textbf{Flat LoCoMo}: Following Memory-R1, we use GPT-4o-mini to extract facts from dialogue turns and build temporal memory banks. We split the QA pairs into train / validation / test following Memory-R1's 1:1:8 ratio.
    \item \textbf{Structured LoCoMo}: We convert the same dialogues into tree-structured memory using the schema: \texttt{Conversation} $\to$ \texttt{Speaker}/\texttt{Session} $\to$ \texttt{Observation}/\texttt{Summary}/\texttt{Event}/\texttt{Turn}. \texttt{Speaker} nodes aggregate persistent observations; \texttt{Session} nodes preserve the temporal record with derived summaries and events.
\end{itemize}

\paragraph{Semantic XPath Domains (OOD).}
Following the evaluation setup in Semantic XPath~\citep{semanticxpath2026}, we use three curated structured memory domains as out-of-distribution test sets:
\begin{itemize}[nosep]
...
We compare six methods, organized into non-RL baselines and RL-trained agents:

\paragraph{Non-RL Baselines.}
\begin{itemize}[nosep]
...
\begin{itemize}[nosep]
    \item \textbf{Zero-Shot (no memory)}: The base LLM (Qwen2.5-3B) answers directly without external memory.
    \item \textbf{Memory-R1 (Flat)}: Our implementation of the Memory-R1 two-agent architecture on flat memory with ADD/UPDATE/DELETE/NOOP, trained with GRPO.
    \item \textbf{Struct Memory-R1}: Our proposed extension to tree-structured memory, where ADD operations require a \texttt{parent\_id} specifying placement in the hierarchy, trained with GRPO.
\end{itemize}
...
Table~\ref{tab:preliminary} summarizes preliminary results across all evaluation datasets. For the three OOD Semantic XPath domains, we report pass rates from~\citet{semanticxpath2026}. RL-trained results (Memory-R1 and Struct Memory-R1) are pending completion of GRPO training.

\begin{table}[h]
\centering
\caption{Preliminary results across evaluation datasets. LoCoMo results use Sub-EM; OOD domain results use Pass Rate. ``/'' indicates results pending GRPO training.}
...
The Semantic XPath baselines on the OOD domains achieve 80--85\% pass rates with significantly fewer tokens than In-Context Memory, establishing strong non-RL reference points. Our RL methods are expected to approach these results on the OOD domains while also being applicable to the LoCoMo conversational setting where Semantic XPath is not directly applicable (flat LoCoMo) or requires manual schema design (structured LoCoMo).
...
Our immediate next step is to complete GRPO training for both Memory-R1 (flat) and Struct Memory-R1 (tree) using Qwen2.5-3B as the base model. Specifically, we plan to:
\begin{enumerate}[nosep]
